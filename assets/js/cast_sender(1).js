...
*/
	...
	var f,
		...
		g = "function" == typeof Object.defineProperties ?
	...
	var ea = function ( a ) {
		...
		ha = "function" == typeof Object.create ?
		Object.create :
		function ( a ) {
			var b = function () {};
			b.prototype = a;
	...
	var ua = Array.prototype.forEach ?
		...
		function ( a, b, c ) {
			for (
				var d = a.length, e = "string" === typeof a ?
					a.split( "" ) :
					a,
					h = 0; h < d; h++ )
	...
	function va( a, b ) {
		for (
			var c = a.length, d = "string" === typeof a ?
				a.split( "" ) :
				a,
				e = 0; e < c; e++ )
			...
			return a.replace( Ja, function ( d, e ) {
				...
				if ( !h ) {
					...
					h = ( e = z ) ?
						...
						h;
					h = new B( h, null, Ea );
					if ( c.tagName && Ha[ c.tagName.toUpperCase() ] )
	...
	chrome.cast.Hb = function ( a, b, c, d, e ) {
		c = void 0 === c ?
			...
			c;
		this.appId = a;
		this.capabilities = Array.isArray( b ) ?
			b :
			[];
		this.requestSessionTimeout = c;
		this.dialRequest = this.language = null;
		this.androidReceiverCompatible = void 0 === d ?
			!1 :
			d;
		this.credentialsData = void 0 === e ?
			null :
			e;
	};
	...
	chrome.cast.media.X = function ( a ) {
		this.containerType = a = void 0 === a ?
			chrome.cast.media.Y.GENERIC_CONTAINER :
			a;
	...
	f.oc = function ( a, b, c, d ) {
		...
		if ( !( 0 > e ) )
			if ( 0 > b )
				d && d( new chrome.cast.Error( chrome.cast.D.INVALID_PARAMETER ) );
			else if ( e == b )
			c && c();
		else {
			var h = null;
	...
	f.bc = function () {
		if ( this.breakStatus && void 0 !== this.breakStatus.currentBreakTime )
			return this.playerState == chrome.cast.media.G.PLAYING && 0 <= this.g ?
	...
	Qa.prototype.next = function () {
		...
		return {
			value: a.done ?
				void 0 :
				this.j.call( void 0, a.value, this.l++ ),
			done: a.done
		};
	};
	var Ra = function ( a, b ) {
	...
	var Ya = function ( a ) {
			if ( a instanceof G || a instanceof H || a instanceof I )
				return a;
	...
	var J = function ( a, b ) {
		...
		if ( 1 < c ) {
			...
			if ( a instanceof J )
				for ( K( a ), c = a.o.concat(), d = 0; d < c.length; d++ )
	...
	var M = function ( a, b ) {
			return L( a.g, b ) ?
				( delete a.g[ b ], = a.size + 1, a.j++, a.o.length > 2 * a.size && K( a ), !0 ) :
				!1;
		},
		K = function ( a ) {
	...
	var hb = function () {
		...
		ib = function ( a, b, c ) {
			var d = a.g[ b ];
			return d ?
				( ( d.status = c ), d.media.forEach( function ( e ) {
		...
		kb = function ( a, b ) {
			...
			if ( c )
				return ( ( c.statusText = b.statusText ), ( c.namespaces = b.namespaces || [] ), ( c.receiver.volume = b.receiver.volume ), c );
		...
		jb = function ( a, b ) {
			...
			if ( a.playerState == chrome.cast.media.G.IDLE && null == a.loadingItemId )
				( a.currentItemId = null ),
				( a.loadingItemId = null ),
				( a.preloadedItemId = null ),
				( a.items = null );
	...
	var sb = {},
		...
		ub = function ( a ) {
			if ( tb )
				...
				void 0 === a && ( a = 0 );
				if ( !S )
					for ( S = {}, c = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789".split( "" ), d = [
							"+/=", "+/", "-_=", "-_.", "-_"
						], e = 0; 5 > e; e++ ) {
	...
	var T = function () {
		...
		U = function ( a ) {
			...
			return ( "object" == b && a ) || "function" == b ?
				"o" + (
					( Object.prototype.hasOwnProperty.call( a, u ) && a[ u ] ) || ( a[ u ] = ++na ) ) :
				b.substr( 0, 1 ) + a;
		};
	f = T.prototype;
	f.add = function ( a ) {
	...
	var V = function () {
		...
		W = function ( a, b, c, d ) {
			...
			void 0 !== d ?
				( b.sequenceNumber = d ) :
				( ( b.sequenceNumber = a.j ), ( a.j = ( a.j + 1 ) % 9007199254740992 ) );
			d = cb( a.B, b );
			c && d && ( a = bb( a.M, b.sequenceNumber ) ) && ( ( b = new chrome.cast.Error( chrome.cast.D.INVALID_PARAMETER, d ) ), ( a = a.g ) && a( b ) );
		};
		...
		a.invisibleSender || ( ( a = new PresentationRequest( X( this ) ) ), a.getAvailability().then( function ( d ) {
			d.onchange = function () {
				...
				c.g.receiverListener(
					d.value ?
					chrome.cast.T.AVAILABLE :
					chrome.cast.T.UNAVAILABLE );
			};
			d.onchange();
		}, function () {
	...
	var Y = function ( a, b, c ) {
		...
		Ab = function ( a ) {
			...
			yb.has( a.type ) && b.ma && (
				b.m ?
				( ( a.sessionId = b.m ), zb( b, null, a.type, a, function () {}, function () {} ) ) :
				b.kb.B( X( b, void 0, "urn:x-cast:com.google.cast.media", a ) ) );
		};
	...
	V.prototype.requestSession = function ( a, b, c ) {
		...
		this.l ?
			wb( b ) :
			( ( c = X( this, c ) ), ( this.l = a ), new PresentationRequest( c ).start().then( function ( e ) {
				Y( d, e, b );
			} ).catch( function ( e ) {
				d.l = null;
				e = new chrome.cast.Error(
					"AbortError" == e.name || "NotAllowedError" == e.name ?
					chrome.cast.D.CANCEL :
					chrome.cast.D.SESSION_ERROR );
				b && b( e );
			} ) );
	};
	...
	var zb = function ( a, b, c, d, e, h, k ) {
		...
		a.Pa( d, function ( l ) {
			l.status && 1 == l.status.length ?
				e && e( l.status[ 0 ] ) :
				( ( l = new chrome.cast.Error( chrome.cast.D.SESSION_ERROR ) ), h && h( l ) );
		}, h, k );
	};
	f = V.prototype;
	...
	V.prototype.onMessage = function ( a ) {
		...
		if ( ( b = bb( this.M, a.sequenceNumber ) ) )
			"error" == a.type ?
			( b = b.g ) && b( a.message ) :
			( b = b.l ) && b( a.message );
		if ( ( b = a.message ) )
	...
	var Eb = function ( a, b ) {
		...
		Gb = function ( a, b, c ) {
			a.g.customDialLaunchCallback ?
				a.g.customDialLaunchCallback( c ).then( function ( d ) {
					Hb( a, b, d );
				}, function () {
					Hb( a, b );
				} ) :
	...
	"complete" == document.readyState ?
		chrome.cast.ha() :
		( window.addEventListener( "load", chrome.cast.ha, !1 ), window.addEventListener( "DOMContentLoaded", chrome.cast.ha, !1 ) );
}.call( this ) );
